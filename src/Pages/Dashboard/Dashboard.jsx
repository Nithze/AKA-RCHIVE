...
export const Dashboard = () => {
	...
	useEffect(() => {
		...
		const fetchShiftData = async () => {
			try {
				const response = await axios.get(
					"http://localhost:5000/api/attendance/current-shift"
				);
		...
		const fetchCesData = async () => {
			try {
				const response = await axios.get(
					"http://localhost:5000/api/employees/ces"
				);
				...
				<div className="content">
					...
					<header className="header">
						<div className="header-left">
							<h1>AKA Management</h1>
							<p>Manage your employees and shipment items swiftly</p>
					...
					<div className="card-container">
						<div className="card-left" ref={cardLeftRef}>
							...
							<div className="card-value">
								<div>
									{cesData.totalSalaryDistribution?.toLocaleString("id-ID") ||
										"0"}
								</div>
							...
							<div className="card-description">
								...
								<div>
									+{cesData.employeeOfTheMonthCount || 0} employee of the month
								</div>
							</div>
						</div>
						{/* end card CES */}
										...
										<tbody>
											{shiftData.map((item, index) => (
												<tr key={index}>
													...
													<td
														className={`presence-col ${getPresenceClass(
															item.attendance[0]?.status
														)}`}
													>
														<div>{item.attendance[0]?.status || "Unknown"}</div>
														<div className="presence-time">
															{item.attendance[0]?.checkInTime || "N/A"}
														</div>
													</td>
													<td
														className={`late-col ${getPresenceClass(
															item.attendance[0]?.status
														)}`}
													>
													...
													<td className="role-col">
														<span className="role-chip">{item.roleName}</span>
													</td>
